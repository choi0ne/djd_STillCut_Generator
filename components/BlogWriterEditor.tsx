...
### TASK 3: 섹션별 일러스트 (6개) - 핵심 내용 요약 기반 프롬프트

⚠️ **절대 금지: 키워드 나열 방식 금지! 반드시 서술형 문장으로 요약하세요!**

**핵심 원칙:**
- 이미지는 **글의 핵심 메시지**를 시각적으로 전달해야 합니다
- 각 섹션의 **주요 내용을 2-3문장으로 요약**하여 전달하세요
- 요약은 **독자가 이해할 수 있는 완전한 문장**이어야 합니다

**프로세스:**
1. 최종 글에서 각 섹션의 핵심 메시지를 파악합니다
2. 해당 내용을 **2-3문장의 서술형 요약**으로 작성합니다
3. 요약은 이미지 생성 AI가 시각적으로 표현할 수 있도록 **구체적이고 명확**하게

**섹션 식별 기준:**
1. Answer First (핵심 결론) - "결론", "핵심", "먼저", "한 줄로" 등
2. Action (즉각적 행동) - "행동", "실천", "할 일", "방법", "루틴", "Top 3" 등
3. Warning (위험 신호) - "주의", "경고", "위험", "레드플래그", "내원" 등
4. The 'Why' (상세 원인) - "원인", "이유", "왜", "기전", "메커니즘" 등
5. Proof (사례와 근거) - "사례", "근거", "연구", "실제로", "경험" 등
6. Closing (요약 및 격려) - "마치며", "요약", "격려", "정리하면" 등

**각 섹션 출력 형식:**
- **sectionNumber**: 1-6
- **sectionTitle**: 영문 섹션명 (Answer First, Action, Warning, The 'Why', Proof, Closing)
- **sectionContent**: ⭐ **핵심 내용 요약 (100-250자)** - 섹션의 핵심 메시지를 2-3문장의 완전한 서술형 문장으로 요약
- **summary**: 1문장 핵심 요약 (30-50자) - 이미지 제목으로 사용 가능한 수준
- **keywords**: 시각적 표현 키워드 3-5개 (이미지에 포함할 요소: 사물, 행동, 감정 등)
- **recommendedPalette**: medical / calm / warm

---
### ❌ 잘못된 예시 (키워드만 나열 - 절대 금지):
\`\`\`json
{
  "sectionContent": "불안, 발작, 심장 두근거림, 호흡 곤란",
  "summary": "공황장애 증상"
}
\`\`\`
...
\`\`\`json
{
  "sectionContent": "공황장애는 갑작스럽게 찾아오는 극심한 불안 발작입니다. 예고 없이 심장이 터질 듯 뛰고, 숨을 쉴 수 없을 것 같은 공포가 밀려옵니다. 하지만 이 증상은 생명에 위협이 되지 않으며, 적절한 관리로 충분히 조절할 수 있습니다.",
  "summary": "공황장애는 갑작스러운 불안 발작이지만 관리 가능합니다",
  "keywords": ["심장 뛰는 모습", "심호흡하는 사람", "안도하는 표정"]
}
\`\`\`
---

**검증 체크리스트 (모든 항목 충족 필수):**
□ sectionContent가 100자 이상의 서술형 문장인가?
□ sectionContent가 완전한 문장(주어+서술어)으로 구성되어 있는가?
□ sectionContent가 섹션의 핵심 메시지를 담고 있는가?
□ keywords가 시각적으로 표현 가능한 구체적 요소인가?
□ 키워드 나열이 아니라 **읽었을 때 의미가 통하는 문장**인가?
